...
function renderCounterCards(data) {
    ...
    onlineCounters.forEach(counter => {
        ...
        const queueFontSize = onlineCounters.length >= 5 ? '1.7rem' : '2.1rem';
        cards.push(`
            <div class="counter-card" style="--q-font: ${queueFontSize}">
                <div class="counter-name" style="font-size: ${queueFontSize}">${counter.name}</div>
...
function renderWaitingList(data) {
    ...
    const items = [];
    for (let i = 1; i <= WAITING_SLOT_COUNT; i++) {
...
async function updateDisplay() {
    try {
        ...
        const waitingList = document.getElementById('waitingList');
        if (waitingList) {
            waitingList.innerHTML = Array.from({length: WAITING_SLOT_COUNT}, (_, i) => `
                <div class="waiting-row">
